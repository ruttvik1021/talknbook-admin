...
const UsersComponent = () => {
  ...
  const pageHeader = () => {
    return (
      <>
        <div className="mb-5">
          <PageTitle
            title={`Users (${isUsersLoading ? 0 : usersList?.data.total})`}
            onClick={() =>
  ...
  if (!usersList?.data.users || usersList.data.users.length === 0) {
    return (
      <>
        {pageHeader()}
        <p>No data found.</p>
        ...
        <TableBody>
          {usersList.data.users.map((user: any) => (
            <TableRow key={user.id} className="border-slate-300 border-b-2">
              ...
              <TableCell>
                <StatusBadge status={user.isActive} type="Active/Inactive" />
              </TableCell>
              <TableCell>
                <StatusBadge status={user.isServiceProvider} type="Boolean" />
              </TableCell>
              <TableCell
